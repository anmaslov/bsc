class ProductsController < ApplicationController
  ...
  # GET /products.json
  def index
    @products = Product.where('id NOT IN (SELECT DISTINCT(product_id) FROM details) AND price > 250 AND supplier_id <> 2 AND is_active = false').order('is_active, updated_at ASC').paginate(:page => params[:page], :per_page => 30)
  end
